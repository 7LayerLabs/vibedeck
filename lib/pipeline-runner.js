    ...
    child.on('close',code=>{
      ...
      try {
        if(step.kind==='codex')output=fs.readFileSync(answerFile,'utf8');
          ...
          if(result.is_error)throw Error(result.result||'Claude reported a failed stage.');
          ...
          if(typeof output==='string' && output.trim() && result.permission_denials?.length)output+='\n\nPermission note: '+result.permission_denials.length+' tool request(s) were denied. Review the result for incomplete work before approving the next stage. Provider permissions were not changed.';
        }
