...
function validatePipeline(value) {
  ...
  return {name:value.name.trim(), steps:value.steps.map(step=>{
    if(!['claude','codex'].includes(step.kind)) throw Error('Automated stages support Claude and Codex. Grok remains available in terminals.');
    ...
    if(model && !/^[a-zA-Z0-9][a-zA-Z0-9._:-]{0,99}$/.test(model)) throw Error('Use a valid model ID, or leave it empty for your CLI default.');
    return {kind:step.kind,role:step.role,model};
  })};
}
...
function executeStage({step,prompt,cwd,resolveCommand,timeoutMs=600000}) {
  ...
  const promise=new Promise((resolve,reject)=>{
    ...
    const args=step.kind==='codex' ? ['exec','--skip-git-repo-check','--sandbox',step.role==='Build'?'workspace-write':'read-only','-o',answerFile] : ['-p','--output-format','json','--permission-mode',step.role==='Build'?'acceptEdits':'plan'];
    ...
    child.on('close',code=>{
      ...
      try {
        ...
        else {const result=JSON.parse(output);if(result.is_error)throw Error(result.result||'Claude reported a failed stage.');if(result.permission_denials?.length)throw Error('Claude needs additional tool permissions. Continue in its terminal to review those requests.');output=result.result;}
...
class PipelineRunner {
  ...
  cancel(){
    ...
    Promise.resolve(run.task?.promise).catch(()=>{}).then(()=>{
      ...
      this.emit({state:'cancelled',text:'Stopped the active process and dropped the remaining stages.',outputs:run.outputs});
    });
  }
}
